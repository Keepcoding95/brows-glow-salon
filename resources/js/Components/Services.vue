      ...
      <div class="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2">
        ...
        <div class="lg:pt-4 lg:pr-8">
          <div class="lg:max-w-lg">
            <h2 class="text-base font-semibold text-indigo-600">Services</h2>
            <p class="mt-2 text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl">
              Îngrijire profesională a sprâncenelor
            </p>
            <p class="mt-6 text-lg leading-8 text-gray-600">
              Alege din serviciile noastre specializate pentru un look natural, curat și elegant. Fiecare serviciu este
...
function whatsAppLink(service) {
  const text = `Bună! Aș dori o programare pentru: ${service.name} - ${service.price} RON`
  return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(text)}`
}

const servicesList = computed(() => props.services || [])

// Map service names to detailed display labels
const displayServices = computed(() =>
  servicesList.value.map((service) => {
    ...
    if (normalized.includes('stilizare forma')) {
      return {
        ...service,
        name: 'Stilizare formă & cosmetizare',
        description:
          'Definire atentă a formei sprâncenelor, adaptată fizionomiei tale, împreună cu cosmetizare completă a zonei pentru un aspect curat și îngrijit.',
      }
    }
    ...
    if (normalized.includes('intretinere')) {
      return {
        ...service,
        name: 'Stilizare întreținere & cosmetizare',
        description:
          'Întreținerea formelor deja create, corectarea firelor crescute și cosmetizare delicată pentru a păstra rezultatul proaspăt, simetric și uniform.',
      }
    }
    ...
    if (normalized.includes('vopsire gel')) {
      return {
        ...service,
        name: 'Stilizare & Vopsire gel',
        description:
          'Stilizare completă a sprâncenelor, urmată de vopsire cu gel/tint profesional, pentru intensificarea culorii și evidențierea liniei naturale a sprâncenelor.',
      }
    }
    ...
    if (normalized.includes('laminare + stilizare')) {
      return {
        ...
        name: 'Laminare & Botox',
        description:
          'Procedură completă de laminare, stilizare și tratament botox pentru fir, care disciplinează firele rebele și oferă volum natural, cu un efect de sprâncene „așezate” și bine definite.',
      }
    }
    ...
    if (normalized.includes('laminare+ tonare')) {
      return {
        ...service,
        name: 'Laminare & Tonare & Botox',
        description:
          'Laminare pentru disciplinare, tonare pentru armonizarea culorii și tratament botox pentru hidratarea profundă a firului, într-un singur pachet complet de îngrijire.',
      }
    }
